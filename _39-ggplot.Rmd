...
knitr::kable(a, digits = 3, longtable = TRUE)
```
...
```{r echo=FALSE}
ggplot(data = a, aes(x = x, y = y, color = Class, shape = Chirality))+
        geom_point(size = 3)
```
...
```{r echo=FALSE}
ggplot(data = a)+
        geom_bar(mapping = aes(x = Chirality, fill = Class), position = "dodge")
```
...
```{r echo=FALSE}
ggplot(data = a)+
        geom_bar(mapping = aes(x = Chirality)) +
        facet_wrap( ~ Class)
```
...
```{r, echo=FALSE}
knitr::kable(yeast_features, longtable = TRUE)
```
